...
export default function ClusterInfoWindow({
  ...
  return (
    <InfoWindow position={stableClusterPosition} onCloseClick={onClose}>
      <div style={{ width: 300, maxHeight: 320, overflowY: "auto", padding: 10, fontFamily: "Plus Jakarta Display" }}>
        ...
        {selectedClusterProperties.map((p) => (
          ...
          >
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: 12, color: "#444", fontWeight: 600, marginTop: 2 }}>{p.title}</div>
              <div style={{ fontSize: 11, color: "#777", marginTop: 2 }}>{p.address}</div>
            </div>
          </div>
        ))}
        ...
        <button
          onClick={onViewAll}
          style={{ width: "100%", marginTop: 8, padding: "8px", background: "#305487", color: "white", borderRadius: 6, fontWeight: 600, fontSize: 12 }}
        >
          View All
        </button>
      </div>
